...
There's a third, rarer one: `EMBARCH_SIGNAL_BAUD` (default `1000000`) sets the line rate when Core reads a DUT signal on its own serial port rather than through the bench. Only relevant once you've declared such a signal and wired a USB-UART bridge to it; leave it alone otherwise. **It has to match what the DUT actually transmits at**, and nothing can check that for you: if they disagree every frame fails its CRC and the capture looks exactly like a firmware bug. Its first real value was `230400`, set 2026-08-27 to match a DUT whose outpost UART runs at that rate, and **raised to `460800` later the same day** when 230400 turned out to be about half what that DUT's tracer needs. That rate is the ceiling of the link rather than a preference — the USB-UART bridge in use refuses to open above 750000 and the nRF54L15 has no baud between 460800 and 921600, so it is the only value both ends can produce. Worth knowing because of what it costs you: 1000000 is this variable's *default*, so a link that could run at 1 Mbaud would need no setting here at all, and one that cannot leaves you keeping two numbers in step by hand — this variable and the DUT's own `current-speed`. **On an installed Core this is not a shell variable** — Core runs as a service, so it reads its environment from the service registration. On Windows that is the `Environment` value (`REG_MULTI_SZ`) under `HKLM\SYSTEM\CurrentControlSet\Services\com.embarch.core`, and the service must be restarted for a change to take.
...
*Older entries archived to [embarch-user-guide.changelog-archive.md](embarch-user-guide.changelog-archive.md).*
