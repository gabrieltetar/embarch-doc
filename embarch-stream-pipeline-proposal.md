...
**What changed.** Scoping [embarch-outpost](embarch-outpost/design.md) — a DUT-side trace UART whose capture would have become the **fourth** near-identical pipeline here — the read direction was accepted and folded into the living docs: [embarch-study-designer/design.md](embarch-study-designer/design.md) §3 decision 39 (`Study.streams`, `StreamTap`/`StreamSource`/`StreamEncoding`, new §4.8, schema 5 → 6), [embarch-core/design.md](embarch-core/design.md) §3 decision 30 (`streams/`, retention, the parameterised route), [embarch-dev-bench/design.md](embarch-dev-bench/design.md) §3 decision 29 (dev-bench stops interpreting payloads; decision 22 marked **superseded**), [embarch-api/design.md](embarch-api/design.md) §3 decision 39, [embarch-ui/design.md](embarch-ui/design.md) §3 decision 10, and a row in [embarch-decision-reversals.md](embarch-decision-reversals.md). One genuinely new source variant the proposal never had, added by the outpost: **`StreamSource::Signal { name }`** — a signal **Core reads itself**, over a wire that bypasses dev-bench, with the carrier resolved by [embarch-topology](embarch-topology/design.md) §3 decision 18 rather than named in the study.
...
- **Does not fix the ~50% DUT connect-and-discover flake rate** ([embarch-dev-bench/milestone-9.md](embarch-dev-bench/milestone-9.md) §6). A tap sits downstream of a working connection and inherits it.
...
| Connect/discover flakiness | ~50%, not root-caused ([embarch-dev-bench/milestone-9.md](embarch-dev-bench/milestone-9.md) §6) | Out of scope, inherited. A working log tap would itself be a good instrument for root-causing it |
...
- 2026-08-25 — **Inbound half accepted and folded into the living docs; outbound half still proposed; status line rewritten to say which is which.** Triggered by [embarch-outpost](embarch-outpost/design.md), whose DUT trace UART would have made four parallel capture pipelines. Two things this proposal did not anticipate: a source that **Core reads itself** rather than dev-bench relaying (`StreamSource::Signal { name }`, with the carrier resolved by topology, not named in the study), and that §10's reserved decision numbers would be spent on unrelated decisions before acceptance — Milestone 6 shipped first and took them, which is recorded here rather than quietly renumbered.

- 2026-08-25 — **Review pass: four open questions closed, and retention specified rather than deferred (new §6.1).** `expect` stays `contains`-only. Retention gets two independent bounds, because a log grows without limit both within a run and across runs: per-tap segment rotation keeping the tail (`EMBARCH_STREAM_MAX_BYTES`, default 32 MiB, two segments, never rewriting a file) with an explicit truncation marker and a new `StreamRef.truncated_bytes`; and a keep-last-N sweep of `study_results/` at `POST /study` (`EMBARCH_STUDY_RESULTS_KEEP`, default 50, `0` disables) — count-based rather than age- or size-based so it needs no clock and stays one knob. This milestone now sequences **before** roadmap Milestone 4. Dev-bench's own `LogLine` gets a reserved `dev-bench` tap in `streams/`, closing §1.1's incidental gap; `dev-bench` becomes a reserved tap name rejected by pre-flight validation.

- 2026-08-25 — **Verified every load-bearing claim against the real sibling repos rather than these docs (§1.1), which corrected two of them.** The little-endian-`f32` assumption is not unwritten firmware as v2–v4 claimed — it is in the shipped `StreamChunkBatch` wire type, together with a `base_utc_ms + i * sample_interval_ms` timestamp model the docs never mentioned and which is structurally wrong for asynchronous notification arrivals; both strengthen the case for the reshape. `StreamChunk` and `StreamChunkBatch` are also both still live (decision 25 described the batch as replacing the single), and Core handles both. Found while checking: `DevBenchMessage::LogLine` already exists and means **dev-bench's own** firmware log, a naming collision worth avoiding — and Core merely `tracing::debug!`s it, so dev-bench's log never reaches a study's results directory at all.

- 2026-08-24 — **v4: the write direction becomes an authored action step, and v3's live console is withdrawn.** `Action::StreamSend { stream_id, payload, expect }` with `StreamExpect { stream_id, contains, timeout_ms }` — the engineer types the string and the expected response at authoring time, dev-bench writes it and runs a raw byte-substring match to produce the step's ordinary `Outcome`, feeding `continue_on_fail` like any other step. Strictly smaller than v3: no mid-study Core→dev-bench message, no concurrent inbound serial in firmware, no write endpoint, no cancel endpoint, and no reproducibility problem — a study stays static and `steps_crc`-sealed. Also sharpened §7's free-text argument, which is now about authored-by-a-human rather than about a live session.
- 2026-08-24 — v3: made the pipeline bidirectional with a live console. Overshot the requirement; withdrawn by v4.
- 2026-08-24 — **v2: generalized from a device-log-specific channel to one generic stream pipeline**, on review feedback that a log and a waveform are the same operation differing only in rendering. Strengthened by two facts checked while revising: nothing has ever streamed a real sample (`embarch-study-designer/design.md` §3 decision 25's own index row; roadmap Milestone 4 exists to do it for the first time), so the reshape is nearly free *until that milestone*; and `embarch-dev-bench/design.md` decision 22's planned little-endian-`f32` firmware decode is an unimplemented DUT-semantics inference that decision 35's principle rules out — deleted rather than built.
- 2026-08-24 — v1: initial draft, in response to "is there a pipeline to get the NUS logs during a study?" — answer: no. Proposed a third capture channel alongside power and sensor-waveform. Superseded.
