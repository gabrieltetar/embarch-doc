...
**Where a link is wanted anyway — `history/` is the standing case — link the index, not the topic file.** `<sub-project>/decisions.md` is a routing table from number to `decisions/<topic>.md`, maintained by whoever splits one, so it survives a mission split; a link straight at a topic file does not. **One gate can see the difference now** (`tasks/doc/027`, 2026-09-09): `check-decision-refs.py` resolves every `[decision N](<sub>/decisions/<topic>.md)` link against the numbers that file actually defines, and fails when it does not hold — `check-links.py` still only sees a path that exists. **The 17 `history/` entries are swept**, repointed at their indexes in the same change; repointing at the *index* needs no routing lookup, which is what made the sweep safe to do mechanically. **One of the 17 was already broken and nobody knew**: `history/umbrella.md`'s decision 43 pointed at `decisions/reporting.md` after 43 moved to `decisions/message-rendering.md`. The remaining 18 links of this shape live inside their own sub-projects, all resolving; the rule above still prefers the bare number to any of them.
