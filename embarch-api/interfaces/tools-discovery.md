...
| `list_targets` | `project` | For `zephyr-west`: every file-backing-validated tuple plus `snippets_by_app`, `default_snippets`, `default_extra_args`. For `static`: exactly one row — **the project itself**, with its configured `build_command`, `chip` and resolved `artifact_path` — because a static project has one target and refuses every selection param ([decisions](../decisions/config-retirement.md) 53) |
