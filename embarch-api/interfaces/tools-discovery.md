...
| `list_targets` | `project` | For `zephyr-west`: every file-backing-validated tuple, each row also carrying `build_dir_name` — that row's own default-build identity, `null` when this project's configured default snippets aren't all available for that row's app ([decisions](../decisions/target-json.md) 77) — plus `snippets_by_app`, `default_snippets`, `default_extra_args`, and `default_target` (the configured base a call naming nothing already resolves to). For `static`: exactly one row — **the project itself**, with its configured `build_command`, `chip` and resolved `artifact_path` — because a static project has one target and refuses every selection param ([decisions](../decisions/config-retirement.md) 53) |
