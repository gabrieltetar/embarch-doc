...
| `[projects.default_target]` | table (`board?`, `variant?`, `revision?`, `app?`) | no | none | `zephyr-west` only — **refused at config load** for a `static` project, which honours no selection at all, and refused when the table is empty. Applied as the **base** selection before a call's own params narrow further, **per field**: a call naming `board` overrides this one's `board` and nothing else. Pins the common single-target case so it does not start erroring the moment a second real target lands in the repo. `list_targets` reports it |
...
| `[[projects.targets]]` | — | — | — | **Retired 2026-09-05** ([../decisions](../decisions/shape.md) 53). A menu nothing ever selected from; a config still declaring one **fails at load naming the retirement** rather than loading a field nothing reads. Declare one `[[projects]]` entry per target instead |
