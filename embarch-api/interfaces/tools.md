...
**Every JSON object this crate emits carries `schema_version: u32`, currently `1`** — the CLI's object, **each NDJSON line** of `study-status --follow` (not only its `summary`), and every JSON MCP result. Bumped by hand on a rename, removal or retype; adding a field is not. **`1` means "the shape as of 2026-09-03", not "unchanged since the beginning"**: the surface moved repeatedly while unversioned. **Not `versions`' `host_type_schema_version`**, which counts study-designer's host types, not this crate's JSON shape.
