...
- `embarch init` against the real healthband repo lands on (or is manually corrected to) an intended real target — `roadrunner` production, not `nff_dev` (§3.3).
...
- Config-only `build`, `flash`, and `build_and_flash` (no `--firmware-path` override) all succeed against physical healthband hardware (§3.6–3.8) — the first time this exact path is proven on real hardware for any project.
...
- **Whether healthband's real workspace toolchain matches `project-a-board`'s venv pattern** is unconfirmed until §3.1 actually runs.
...
- **`build_cwd`/`artifact_path`/`artifact_path_for_core` for the real healthband workspace** are unconfirmed — Milestone 1 found `project-a-board`'s assumed value wrong only by attempting a real build; the same could happen here (§3.2/§3.6).
