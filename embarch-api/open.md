...
- **The smoke harness ([decisions](decisions/tests.md) 30) is named and unwritten.** The six mocked criteria beside it live in `tests/` ([decisions](decisions/tests.md) 46), with one gap left: the end-to-end half is `#[cfg(unix)]`, so Windows runs the direct tests only. The gate's blind spot over `crates/embarch-core-client`'s own 28 unit tests is closed — it is a workspace member ([decisions](decisions/tests.md) 56) — but **that was found by hand, not reported**, and nothing yet says whether another crate in the suite sits behind the same one. The bearer half is closed — the sweep is derived from the client's source ([decisions](decisions/tests.md) 54) and one funnel applies the token ([decisions](decisions/core-link.md) 55).
