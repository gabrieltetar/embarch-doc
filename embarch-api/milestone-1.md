...
- 2026-07-20 — Initial draft, scoping embarch-api's half of Milestone 1 (Flash).
- 2026-07-20 — Revised §3.5 (now §3.6) to build the west-runner wrapper on top of embarch-api's new CLI subcommands (`design.md` §3.10/§5a) instead of a from-scratch Core-calling implementation; added new §3.4 (implement the CLI subcommand surface itself, all six subcommands) and renumbered the former §3.4/§3.5 to §3.5/§3.6; updated §1, §2, §4, §5 to match; added the open question of whether the west-runner wrapper is still worth keeping.
- 2026-07-20 — Linked the new `milestone-1-implementation-guide.md` from the header — it turns §3.3/§3.4 into ready-to-run agent prompts against the real embarch-api source tree.
- 2026-07-20 — §3.3 (`artifact_path_for_core`) and §3.4 (CLI subcommand surface, `cli.rs`) shipped via `milestone-1-implementation-guide.md`'s Prompts 1–4; durable record now lives in `design.md` §4, §5a, §10, §13. §3.1 (west/venv PATH), §3.5 (end-to-end hardware validation), and §3.6 (west-runner wrapper) remain open.
- 2026-07-21 — §3.2 (config TODOs) resolved against real hardware: `chip = "nRF54L15"`, `base_url` set to the confirmed WSL2 host-gateway IP. §3.5 partially validated: `embarch-api flash project-a-board` (CLI, explicit `--firmware-path`) successfully flashed a real `west`-built artifact onto the physical nRF54L15 board through Windows-hosted Core — the first real end-to-end proof of this milestone's whole point. That exercise also surfaced two real bugs, both recorded in `design.md` (§7, §9, §12) and `embarch-core/design.md`: Core's `internal_err` was silently discarding the real error behind every `500` response (now fixed), and §3.3's proposed `artifact_path_for_core` value for `project-a-board` assumed the wrong build-output location (`west build` actually writes to the workspace root, not under `build_cwd` as configured) — the field remains unpopulated in the real config until that's sorted out. `list_projects`/`status`/`build`/`build_and_flash`/`reset`/`serial_log` and the MCP path remain unvalidated (§3.5).
