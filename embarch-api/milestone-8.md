...
- 2026-08-19/20 — **§3.7/§3.8 closed — every DoD item now done.** `study_power_data`/`study_waveform_data` confirmed cleanly reporting "no data" via MCP (§3.7). §3.8's MCP-path repeat found and fixed a real bug (`design.md` decision 31): `run_study`'s schema didn't declare `study` as an object, so at least one real MCP client sent it JSON-encoded as a string, failing deserialization — fixed via an explicit schema override plus a handler-side string-fallback, then live-validated end to end via a hand-rolled MCP stdio client against the real Core + ESP32-C5 (this session's own already-connected MCP server predates the fix, so a fresh process was used instead of waiting on a reconnect). The same investigation reproduced `embarch-study-designer/design.md` §7's flagged stack-overflow risk for real in a debug build, and confirmed a `--release` build doesn't hit it.
- 2026-08-19 — §3.6 confirmed done: `embarch-api run-study --study-file self_test_study.json` ran the self-test study to `"completed"` against the real Core + the real ESP32-C5 dev-bench board — Milestone 2's target board as of [embarch-dev-bench/milestone-8.md](../embarch-dev-bench/milestone-8.md)'s 2026-08-19 retarget from the nRF54L15DK ([embarch-decision-reversals.md](../embarch-decision-reversals.md) row 13). §3.7/§3.8 remain open.
- 2026-08-18 — §3.1–3.5 implemented for real: `core_client.rs` gained the 4 new endpoint methods, new `src/study.rs` holds the shared `steps_crc` recompute helper, `tests/fixtures/self_test_study.json` added. `cargo build`/`test` (49 passed)/`clippy -D warnings` clean. `design.md` updated to match. §3.6–3.8 (live runs) not started — no real Core/dev-bench to run against yet.
- 2026-08-18 — Confirmed: `study_power_data`/`study_waveform_data` stay in this milestone's scope alongside `run_study`/`study_status` (§1's "judgment call" flag resolved, no change in plan — user confirmed "ok for now").
- 2026-08-18 — Initial draft, scoping `embarch-api`'s half of Milestone 2 (Dev Bench Self-Test) — added to this milestone's scope during its design-questions pass, expanding the roadmap's original Core+Dev-Bench-only framing to include the already-locked-in `run_study`/`study_status` surface (`embarch-study-designer/design.md` §6), plus `study_power_data`/`study_waveform_data` for surface symmetry with Core's full four-endpoint implementation.
