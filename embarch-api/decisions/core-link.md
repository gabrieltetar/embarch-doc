...
Originally scoped to trigger only for a genuinely remote Core, on the theory that local and WSL2-host topologies always share a filesystem. **That premise was false for WSL2 specifically**, when Core runs as the installed Windows service rather than in the foreground. Both WSL2-host and remote now upload bytes; only a same-machine or explicitly-addressed Core keeps sending a path. `artifact_path_for_core` and its UNC computation are **fully retired rather than left unchanged as originally planned** — see [../spec.md](../spec.md) §4 for the Session 0 mechanism and why keeping them stopped making sense.
