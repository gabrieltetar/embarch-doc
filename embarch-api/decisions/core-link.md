...
Originally scoped to trigger only for a genuinely remote Core, on the theory that local and WSL2-host topologies always share a filesystem. **That premise was false for WSL2 specifically**, when Core runs as the installed Windows service rather than in the foreground. Both WSL2-host and remote now upload bytes; only a same-machine or explicitly-addressed Core keeps sending a path. **Every earlier "confirmed working" claim for the UNC mechanism had been validated against a *foreground* Core, never the installed service.** `artifact_path_for_core` and its UNC computation are **fully retired rather than left unchanged as originally planned** — see [../spec.md](../spec.md) §4 for the Session 0 mechanism and why keeping them stopped making sense.
...
The fallback chain gained a final step before erroring. **The correction matters more than the mechanism:** this decision originally justified it by calling a DUT's own serial console "a different, project-specific port that config already covers" — treating DUT-UART capture as a real intended use on par with the bench's. Investigating a real empty capture found that premise was **never true**: Core's serial link was only ever meant to reach the bench. A project's `serial_port` field existing does not mean pointing it at a DUT was ever a supported goal, just that nothing in code stops it.

### 36 — The whole runtime moves onto a dedicated 512 MiB-stack thread
The first real GATT-sized status call reproduced a known deserialization stack overflow **in production** rather than in a test or a tiny fixture, crashing the live MCP server repeatedly — each crash silently respawning a fresh process, so the client saw only a closed connection.

**The first fix attempt did not work, and why matters:** `Builder::thread_stack_size` only sizes threads the *runtime* spawns. The top-level future driven by `block_on` runs on whatever thread calls it, which for `#[tokio::main] fn main` is the process main thread, at the OS default, **with no knob to change it**. Confirmed by reproducing the crash with a 64 MiB runtime setting still in place: the panic named `main`, not a worker.

So `main` spawns the runtime on a thread it sizes itself — the same idiom this crate's own tests already used for the identical cause, now applied to the real binary. 64 MiB was empirically insufficient against a real payload; 512 MiB was needed. **It incidentally closed a separate open item that only looked unrelated:** a release build overflowing on any subcommand, even `list-projects`, was the same non-resizable-calling-thread issue, triggered by release-mode inlining collapsing frames differently. One fix, two bugs.

### 37, 38 — `embarch-core-client` extracted, and given the two wrappers nothing here needed
`embarch-ui` needed to reach Core exactly as this crate does — bearer injection, per-call timeouts, the topology-branched flash transport, typed error mapping — so the choice was between it duplicating that logic or **both depending on one implementation**. The latter follows the suite's now-repeatedly-established one-implementation-many-call-sites shape rather than reintroducing the mirrored-copy risk that shape exists to eliminate. A plain path dependency, not a Cargo workspace, matching how this repo already depends on its sibling crates.

**Real dependency-footprint cleanup fell out for free:** this crate no longer depends on `bytes` or `reqwest` directly, and — since a default-port constant was its last use of it — no longer on the topology crate either, confirmed by `cargo tree` showing neither `probe-rs` nor `serialport` anywhere in the graph.
...
Same argument as decisions 37/38: `embarch-ui` reaches Core the same way this crate does, and a second SSE implementation there would be the mirrored-copy risk that extraction exists to remove. It also buys testability without widening this package's own `lib` surface ([decisions](shape.md) 46) — the decoder and the follow loop are `pub` in a library crate `tests/` can already reach, so nothing had to move out of the binary.
...
**The correction on implementation: per-user, not machine-wide.** The machine data dir works for Core, which runs as a service; it does not work here, because this runs as the engineer and `/var/lib` is root-owned — verified rather than assumed. Both alternatives were worse: hard-failing makes logging depend on a one-time `sudo`, and "machine dir if writable, else per-user" is a runtime probe that can land the writer and the reader in **different places**. Nothing is lost, since single-engineer scope means there is no second user to be machine-wide for.

**Both modes had to be given something to log:** a one-shot CLI run emitted nothing at all, since no subcommand calls the tracing macros on its own, so the file would have stayed empty for exactly the invocations this decision names. Two lines now bracket every run. **And two defects, one in Core's file rather than this one:** Core tees a single ANSI-coloured stream to both stderr and its logfile, so every line in the deployed log carries escape sequences, which a UI had been rendering as literal garbage since it shipped. This crate writes two layers instead of one teed writer — and the two-layer form has **no filter of its own**, so the first build wrote `hyper` and `reqwest` trace output into the file. Caught by reading the file, not by a test.
