...
Decisions 37/38 settled the shared Core client is one implementation; neither settled where it lives. It sits at `crates/embarch-core-client`, inside this repo, unlike this suite's other two shared crates: `embarch-study-designer` (decision 8) and `embarch-topology` (decision 13) each got a standalone repo, both times because each has independently-versioned consumers on both sides of a hardware/software split or an FFI/C boundary — decision 8 names "three independently-versioned consumers … two Cargo dependents, one FFI/C consumer" as the case a standalone shared crate is for. This crate has two consumers, both plain Cargo path dependents, no FFI boundary — that case does not hold here. But the question was never taken up either way: decisions 37/38 extracted the crate out of this repo's own `core_client.rs`/`config.rs`/`token_discovery.rs` and left it where the extraction happened, rather than moving it to a tenth repo the way the other two shared crates were given one.
