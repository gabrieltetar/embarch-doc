...
For a Zephyr build the **first** compiler error is usually the actionable one and everything after it is cascade, so an early failure followed by `cmake` and `ninja` grinding on can scroll the only useful line out of a tail-only cap entirely. First 16 KB, last 48 KB, the middle dropped behind a marker naming how many bytes went and how many were kept at each end. Unchanged below the cap.
...
- **The cap bounds the retained bytes, not each half.** Head plus tail is at most `OUTPUT_CAP_BYTES`, and the marker sits on top of it exactly as the old one did — so this buys a head without doubling a response an MCP client has to carry. The alternative, a cap per half, is the same constant meaning two different things depending on whether a log overflowed.
