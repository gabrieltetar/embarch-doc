...
how it parses an older Core, one WSL2 predicate it delegates, and where the crate itself lives.
Split out of [core-link.md](core-link.md) on 2026-09-11 (`api/067`), which went over its 12,288 B
cap — a topical seam (address resolution vs. the crate that does the addressing), not a squeeze.
...
Later additions followed the same rule and exposed its cost: the signal-route wrappers are **mirrors** of the topology crate's types rather than those types, because the real ones sit behind the feature that links `probe-rs`, and this crate never links hardware. That leaves a coupling **no crate in the suite can typecheck**, so it is pinned from each side against the same JSON literal, each test naming the other. **Both mirrors are now pinned** (`api/066`, `tasks/core/024`): `client.rs`'s doc comments on the alert and enrolled-board wrappers name `embarch-core`'s `alert_round_trips_against_the_client_s_pinned_shape` and `enrolled_board_round_trips_against_the_client_s_pinned_shape` (`embarch-core/src/api.rs`), each test naming the other side.
