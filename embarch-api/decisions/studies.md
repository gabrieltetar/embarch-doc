...
**Every real bug found in this project to date came from a live run** against a real Core or a real repo — decision 12's whole implementation history is nothing but that — not from the still-unwritten unit-test suite. That is a real, working methodology, just an unnamed and unrepeatable one. So it gets a name and a script: a throwaway Core instance plus a synthetic fixture repo, re-running a fixed sequence of calls. Not a substitute for the mocked unit tests, which remain the acceptance criteria below the process boundary — a **repeatable** version of what the changelog already shows being done by hand.

### 31, 33 — `run_study`'s schema declares an object, and the handler tolerates a stringified one
*(One decision recorded twice. See the tombstone note at the end of this file.)*
...
**A failure mode the CLI path structurally cannot hit**, since `--study-file` parses the file directly with no schema involved — which is why the milestone asked for an MCP-specific repeat rather than trusting CLI coverage to imply MCP coverage. **Fixed two ways:** the schema is overridden to declare an object, for clients that read the type; and the handler unwraps a string by parsing it as JSON first, for those that do not.
...
`study_stream_data` replaces the three fixed per-channel tools, mirroring Core's collapse of three routes into one parameterised one. **The three stay as aliases for one release** rather than breaking an agent's working invocation mid-flight. `list_study_streams` names what a study actually captured, since an agent that must *guess* a tap name to read one has been handed a worse tool than it had. Three things that settled on implementation:

- **`list_study_streams` needed no new Core route, and confirming that was the work.** The study endpoint already returns the whole result inline, so the tool is a projection of a call that existed. A listing endpoint Core does not need is exactly the surface this suite keeps deciding not to build.
...
Not design questions — the surface having been extended past the code that implements it. Recorded because **the tool text still described the old behaviour, which is what a caller reads**.

**(a) A DUT reflash flashed and did not reset.** The bench half resets explicitly, with a comment saying why; the DUT half fell straight through to submitting the study. So a study asking to run against freshly-built firmware ran against **whatever was already on the board**, and reported a successful reflash while doing it. This is exactly the coupling the dev-workflow doc records as a hazard for hand-driven flashing — *"it presents as 'I flashed it and nothing changed'"* — written up and then left sitting inside the one call whose whole purpose is to remove hand-driven flashing. Found by reading the code before trusting it. Fixed.
...
**Tombstone: 31 and 33 are the same decision, recorded twice.** Their text is byte-identical. The duplicate was created in the very commit that **deleted** decision 31 — someone re-added the identical content under a new number — which is why the deletion went unnoticed for eight days while three other docs went on citing 31. Both numbers resolve here; neither is reused for anything else.
