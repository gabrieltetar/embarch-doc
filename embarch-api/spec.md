...
- **Truncation keeps the tail only** — the last 64 KB behind a marker naming the cap, cut on a UTF-8 boundary because an offset inside a codepoint panics `String::replace_range`. Head-and-tail was the intent, never built: [open.md](open.md) carries it.
