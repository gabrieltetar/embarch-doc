...
**Tier 2 — writes real, shared machine state:** any *live* invocation of `embarch setup`, `embarch up`/`down`, `embarch setup --uninstall`, or `embarch-core`'s own `install`/`start`/`stop`/`uninstall`. These touch a real OS service, the real per-user `PATH` (registry or rc files, §3), and a real system-wide token file (`/var/lib/embarch/token` on Linux, `%ProgramData%\embarch\token` on Windows — **not** redirectable by `HOME`/`XDG_DATA_HOME`, since it's a fixed system path, not per-user). Concretely, on a real `wsl-host` machine: `setup --uninstall`'s token-removal step resolves to `/mnt/c/ProgramData/embarch/token` — the real Windows-hosted Core's real token file, with no env-var override that redirects it anywhere else.

**On the repo owner's own real daily-use machine, this is explicitly authorized — no need to ask first (2026-08-17)**, recorded in `embarch-core`/`embarch-umbrella`'s own `CLAUDE.md`. Elsewhere — a different machine, a different person, CI, or anything where that standing authorization doesn't clearly apply — the suite's firmware build/flash rule is the right default to fall back to: ask before running a Tier 2 command live, unless it's provably running inside a disposable environment built for exactly this, in which case asking isn't needed because nothing real is at stake:
...
- 2026-08-17 — §5 updated same day: the repo owner explicitly authorized running Tier 2 commands live against their own real daily-use machine directly, no need to ask first there — recorded in `embarch-core`/`embarch-umbrella`'s own `CLAUDE.md` too. The firmware build/flash rule remains the fallback default for any other context (a different machine/person, CI) where that authorization doesn't apply; it's otherwise unchanged by this.
