...
**On the repo owner's own real daily-use machine, this is explicitly authorized — no need to ask first (2026-08-17)**, recorded in `embarch-core`/`embarch-umbrella`'s own `CLAUDE.md`. (The separate firmware build/flash rule this section originally pointed to as the fallback default has itself been removed the same day, globally — build/flash autonomy is now the default on any firmware project too.) Elsewhere — a different machine, a different person, or anything where the owner's own standing authorization doesn't clearly apply — ask before running a Tier 2 command live, unless it's provably running inside a disposable environment built for exactly this, in which case asking isn't needed because nothing real is at stake:
...
- 2026-08-17 — Same day, later: the separate firmware build/flash rule §5 had pointed to as the fallback default for other contexts was itself removed globally (any project, build and flash both) — updated §5 to stop citing a rule that no longer exists; `embarch-api/milestone-7.md`'s "ask before this step" gates on the real healthband flash steps removed to match.
