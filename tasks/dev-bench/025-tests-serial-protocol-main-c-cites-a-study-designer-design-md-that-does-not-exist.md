# 025 — `tests/serial_protocol/src/main.c` cites an `embarch-study-designer/design.md §3` that does not exist

**State:** blocked — re-scoped into `tasks/dev-bench/022`, which covers this file and two others in
one pass (leg 106, 2026-09-13). **Unparks when** `022`'s unit lands: if it covered
`app/tests/serial_protocol/src/main.c` this task is `done` and retired in that unit's fold; if it
did not, this task goes back to `open` with the remaining count. Not dispatchable meanwhile — one
task per sub-project is per slot, so running it separately would spend a second leg's unit on the
same sweep. **Note the path below is wrong**: the file is at
`app/tests/serial_protocol/src/main.c`, so this task's own `Done when` grep would have returned
zero against a path that does not exist. `022` carries the corrected path.
**Source:** split out of `dev-bench/019`, 2026-09-12.
**Scope:** dev-bench
**Hardware:** none
**Owner:** no

## What

`embarch-dev-bench/tests/serial_protocol/src/main.c` has **28** citations of the dead
`embarch-study-designer/design.md §3` filename (same defect class as `dev-bench/019`,
which fixed `eap.h`/`eap_interp.h`/`eap_interp.c`). `019` confirmed only the filename is
dead and the counts, by mechanical grep — **the per-hit decision numbers in this file have
not been checked** against `embarch-study-designer`'s `decisions.md` index. Say so again in
whatever unit picks this up if any number turns out not to resolve.

## Why now

Same defect class cleared out of `embarch-ui` by `ui/033`/`ui/039`, out of `embarch-core` by
`core/008`, and out of the `.eap` trio by `dev-bench/019`. No gate can see it:
`check-decision-refs.py` resolves decision numbers in `*.md` under a repo root only, and
these are C comments.

## Done when

- [ ] `grep -n "design\.md\|milestone-" tests/serial_protocol/src/main.c` returns zero.
- [ ] Each becomes `` `embarch-study-designer` decision N `` — the cross-repo form settled in
      `api/052` — with N **unchanged** and confirmed resolvable in that repo's `decisions.md`.
- [ ] Host-side checks green; say plainly what could and could not be run (this suite's own
      `tests/unit` ztest note does not apply here, but any Zephyr-test-runner build
      constraints for this directory should be recorded the same way if they turn out to
      apply).
