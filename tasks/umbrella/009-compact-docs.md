...
**Scope:** umbrella
**Hardware:** none
**Compacts:** embarch-umbrella/decisions/bind.md

**`decisions/doctor.md`, `open.md` and `spec.md` were struck off this line on 2026-09-09.** All
three were paid and each is accounted for in the `**Reserve:**` paragraph below, which stands
verbatim; `spec.md` and `open.md` are back in reserve today and filed against
`tasks/umbrella/038-compact-umbrella.md`, not this task. They had been left on the line wrapped in
`~~…~~`; **that line is data** and `check-task-state.py` now fails on a struck-through entry,
because leg 057's richer strike-through of the same shape stopped `check-doc-size.py` recognising
the line at all and failed the gate.
**Size debt due:** 2026-10-06
...
- [x] **`decisions/doctor.md`'s item is closed, `umbrella/037`, 2026-09-07 — 11,346 → 10,687 B, out of reserve at 87.0%.** Paid the way `020` and `022` paid this file: **by splitting, not squeezing.** Decision 19 — check 13's own mission, firmware freshness on the dev bench — moved verbatim into a new [`decisions/dev-bench-firmware.md`](../../embarch-umbrella/decisions/dev-bench-firmware.md), and `037`'s own new reasoning (decision 47) was filed there rather than into the reserve it would otherwise have spent. **A split restates nothing, so this task's `In flux: yes` did not forbid it** — the third time that reasoning has been used on this exact file, after `020` (decision 22 → `bind.md`) and `022` (decisions 10 and 12 → `integration.md`). The seam was checked before it was cut: no inbound link to decision 19 lives outside `embarch-umbrella`/`embarch-doc`. **Three of the four items are now closed and only `decisions/bind.md` (11,409 B, 92.8%) is left**, untouched since `umbrella/021` — so what remains of this task is one file and the parked pass, not four.
- [x] `open.md` out of reserve, `umbrella/023`, 2026-09-06 (4,527 B, 88.4%); **`decisions/doctor.md` went back in the same unit, 6,188 -> 11,095 B (90.3%), and its item is open again.** `spec.md` (9,154 B, 89.4%) is out and is the only one of the four that is. **`decisions/bind.md` went back in with `umbrella/021`, 2026-09-06 and is untouched at 11,409 B (92.8%); `open.md` was paid out by `umbrella/023` and its item is closed** — see **Reserve** above for what each spent it on and why `open.md`'s 40 remaining bytes make a ride-along impossible next time.
