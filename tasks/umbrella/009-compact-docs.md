...
- [ ] All four files out of reserve. `spec.md` (9006 B, 87.9%) and `decisions/doctor.md` (6188 B, 50.4%) are out and stay out. **`open.md` and `decisions/bind.md` went back in with `umbrella/021`, 2026-09-06; `umbrella/022` took `open.md` to 4,661 B (91.0%) and `bind.md` is untouched at 11,409 B (92.8%)** — see **Reserve** above for what each spent it on and why `open.md`'s 40 remaining bytes make a ride-along impossible next time.
