...
- **Whether probe-rs writes are what killed the nRF54L15 is evidence, not proof** (decision 36). One erase-and-program over J-Link recovered a board that had been unresponsive across a whole session, which is the strongest available evidence — but **the counterfactual is not established**: something else may have been cleared by a full erase-and-reprogram that happened to work.
- **Discarding a signal port's buffered input on open is not sufficient** (decision 30). With the clear in place a capture still began with **18 stale records** carrying a cycle count seconds from the rest, and the purge reported no error — so the bytes are presumably still inside the USB-UART bridge or in flight, beyond an OS-level buffer purge. The clear stays, being correct and free. The working defence today is `embarch-ui` refusing the DUT clock when a capture's two clocks contradict each other, which costs the microsecond axis for the whole capture. **Candidate fix:** drop a leading run of records whose cycles are discontinuous with the bulk, at render time, where the whole file is in hand and the discontinuity is unambiguous.
...
- **`EMBARCH_TOKEN` is a single shared static token, not per-caller credentials**, and there is no TLS. Source of truth: [embarch-token.md](../embarch-token.md) §8.
...
- **The nRF54L15 hardware-ID register address** and the end-to-end validation of the moved identity gate are `embarch-topology`'s open questions since decision 22's move, not this repo's. Relocating the code changed nothing about whether that address is confirmed against real silicon.
