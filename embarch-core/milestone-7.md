...
- Core is confirmed running and reachable from WSL2 at the address embarch-api's healthband config actually uses (§3.1).
- The healthband board's physical probe is confirmed visible to probe-rs on Windows (§3.2).
- The live-discovered target's SoC resolves to the correct probe-rs chip string via `/resolve-chip` (§3.3).
- `/status`, `/reset`, `/serial-log` all succeed against the real healthband board from WSL2 (§3.4).
- `/flash` succeeds against the real healthband board — proven via `embarch-api/milestone-7.md` §3.7–3.9's config-only CLI and MCP calls, not a standalone Core-side check.
- No `usbipd attach`/`usbipd bind` anywhere in the path (same invariant as Milestone 1).
- Any healthband-specific gap found (probe type, port assignment, chip resolution) folded back into `design.md` per DOC-PROTOCOL.md §5.

## 5. Open questions / risks carried into execution

- **Probe type for the healthband board is unconfirmed** — could be a different enumeration path on Windows than `project-a-board`'s J-Link if it turns out to be a DK's onboard debugger instead (§3.2).
- **Which `COM<N>` port the healthband board's UART/CDC lands on** is unconfirmed until §3.4 runs (expected to differ from `project-a-board`'s `COM5`, not a bug if it does).
