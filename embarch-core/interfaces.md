...
- **`404` is often an expected state, not a failure:** no dev-bench port (bench unplugged), no board enrolled under a role, a study with no `streams/`, an unmapped SoC. `502` means the bench itself is the problem. **`503` is not in Core's vocabulary at all** — no handler emits one, and a request meeting a held `hw_lock` queues rather than being refused (decision 14, `tasks/core/013`); a caller that branches on `503` is branching on a status it will never see.
