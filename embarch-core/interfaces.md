...
| `GET` | `/study/{id}` | — | `{status: "pending"\|"running"\|"completed"\|"failed", current_step?, total_steps?, result?, reason?}`. **`current_step` is the 0-based index of the last step that *finished*** — absent until one has, and `total_steps - 1` on a completed run, so `"step {current_step} of {total_steps}"` reads one short at the end and a caller wanting "2 of 2" adds one itself (decision 43). `result` is `events.json` read off disk on demand — never a resident copy (`StudyResult` is ~1.3 MB by type). `404` for an unknown id, **including one from before a restart** — indistinguishable by design |
