...
- **Errors are plain text, not JSON**, on every non-2xx: the full `anyhow` chain (`{e:?}`, not `{e}` — `Display` silently dropped every underlying cause). Parse JSON only on 2xx. `internal_err` also logs the same chain server-side, so a failure is visible in `core.log` even if the caller drops the body. A `{code, message, cause}` JSON body is designed and **not built** (decision 12).
...
|---|---|---|---|
| `GET` | `/status` | — | `{ status: "ok", probes: [ProbeInfo], study_designer_schema_version: u32 }`. The version is `embarch-study-designer`'s **host type** constant, the one guarding this hop. `core_version`/`contract_version` are designed and unimplemented (decision 13). `probes` re-enumerates fresh on every call — cheap descriptor enumeration, no attach, which is why a UI can poll it |
...
| `GET` | `/study/{id}/events` | — | SSE. One JSON `StudyEvent` per frame (`StepCompleted`/`SampleBatch`/`StatusChanged`), pushed as Core processes it, filtered to this `study_id`. A slow subscriber gets `event: lagged` rather than silently missing messages |
