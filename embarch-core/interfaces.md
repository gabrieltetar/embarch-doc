...
- **`404` is often an expected state, not a failure:** no dev-bench port (bench unplugged), no board enrolled under a role, a study with no `streams/`, an unmapped SoC. `502` means the bench itself is the problem. **`503` carries two distinct meanings.** Everywhere except `/validate`, it means `hw_lock` was contended and the wait timed out (decision 14, built `tasks/core/013`): a caller that meets the lock already held waits up to 500 ms, then gets `503` with a plain-text body naming the route currently holding it (e.g. `hw_lock held by POST /flash; POST /reset refused after waiting 500ms`) — a caller can branch on this status and does not need to guess whether Core is hung. **On `POST /validate`, `503` instead means the enrolled probe couldn't be opened at all** (`kind: "not_attached"`, decision 59) — a condition with the opposite correct response (wait for a cable, not for a lock to free up); a caller reading this status has to check which route answered before deciding which meaning applies.
