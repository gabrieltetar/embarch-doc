...
| `POST` | `/validate` | `{role}` | `200 {ok: true, …}` on a match; `409` with `{kind: "mismatch", recorded_hardware_id, live_hardware_id: Some, reason, fix_it_url}` when a live ID was read and disagrees; `503` with `{kind: "not_attached", live_hardware_id: null, fix_it_url: null, …}` when the enrolled probe was not found among attached probes at all (unplugged, most likely) — decision 59: distinguished by `kind`, never by parsing `reason`. **Not** the same as a probe that *is* found but fails partway through the check (`.open()`, target-power, attach, hardware-ID read): that case is a plain `500`, un-downcastable, indistinguishable from any other internal failure — decision 59's amendment. `404` if nothing is enrolled under `role`. The explicit, non-destructive counterpart to the re-check `flash`/`reset`/`POST /study` already run mid-attach |
