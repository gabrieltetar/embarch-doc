...
| `POST` | `/validate` | `{role}` | `200 {ok: true, …}` on a match; `409` with `{kind: "mismatch", recorded_hardware_id, live_hardware_id: Some, reason, fix_it_url}` when a live ID was read and disagrees; `503` with `{kind: "not_attached", live_hardware_id: null, fix_it_url: null, …}` when the enrolled probe couldn't be opened at all (unplugged, most likely) — decision 59: distinguished by `kind`, never by parsing `reason`; `404` if nothing is enrolled under `role`. The explicit, non-destructive counterpart to the re-check `flash`/`reset`/`POST /study` already run mid-attach |
