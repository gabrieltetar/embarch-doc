...
| `POST` | `/validate` | `{role}` | `200 {ok: true, …}` on a match; `409` with `{kind: "mismatch", recorded_hardware_id, live_hardware_id: Some, reason, fix_it_url}` when a live ID was read and disagrees; `503` with `{kind: "not_attached", live_hardware_id: null, fix_it_url: null, …}` — decision 59, distinguished by `kind`, never by parsing `reason`. **`"not_attached"` covers six causes as of `embarch-topology` decision 34 (`core/077`, decision 59's second amendment):** the enrolled probe not found among attached probes at all (unplugged, most likely), *and* one found but failing partway through the check — `.open()`, target-power, attach, core-select, or the hardware-ID read. All six set `live_hardware_id: None`, none is an identity question, and none gets `fix_it_url`; only `reason`'s own text (echoed in the JSON body) still says which of the six it was — considered and declined as a fourth wire distinction, since nothing in this suite branches on it structurally. `404` if nothing is enrolled under `role`. The explicit, non-destructive counterpart to the re-check `flash`/`reset`/`POST /study` already run mid-attach |
