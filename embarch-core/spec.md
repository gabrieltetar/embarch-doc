...
- **One `hw_lock` serialises all hardware access**, held for the whole handler body. **Contention queues; it does not refuse and it names no holder** — a second caller waits for the first to finish, with no deadline and no message [measured 2026-09-06, three concurrent `/serial-log` on one port, all `200`, 3× the single-call hardware time]. Decision 14's `503` naming the holder is designed and not built (`tasks/core/013`). A separate `study_lock` serialises studies; a signal tap's read-only port takes neither.
...
Split into [interfaces/constants.md](interfaces/constants.md) on 2026-09-09
(`tasks/core/030`) when this file reached its size cap — every value, its
`[measured]`/`[assumed]` tag and its provenance note moved verbatim, nothing
cut. That file also has the on-disk path convention (`%ProgramData%\embarch\`
on Windows, `/var/lib/embarch/` elsewhere).
