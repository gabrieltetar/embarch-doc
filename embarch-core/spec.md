...
| `logs.rs` | `latest_log_file`/`tail_lines`/`FollowState`, one implementation behind the CLI and both HTTP routes. `FollowState` publishes whole lines only — its offset advances past a `\n` or not at all (decision 44) |
