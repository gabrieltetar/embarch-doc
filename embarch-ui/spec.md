...
| **Topology** | One diagram carrying both of a role's bindings (45): its **board type**, clicked to pick, and its **probe**, dropped on (43). Plus **signal routing** (the one human surface for declaring a DUT signal's route), the project's **board-type catalog**, **saved benches**, and **Validate topology**, which replaced the alert list (44) |
