...
- **A board type's row is its build menu, and a DUT is picked as a combination the scan really reports** (47): revisions and variants per row — **not apps, the study's axis** (49) — the pinned one marked and a pin the scan no longer backs marked as stale. Never a cross product: Zephyr backs a pair only where a file does. The board type goes to Core; **the combination goes to `embarch/boards.toml` and Core is never told a revision**.
...
- **The Topology tab explains nothing it can show** (50): a path or a state is the only note.
