...
| **Topology** | The board/probe diagram, **signal routing** — the one human surface for declaring a DUT signal's route — **enrolling** by dropping a probe onto the box that says a role is empty (43), the project's **board catalog**, **saved benches**, and **Validate topology**, which replaced the alert list (44) |
...
- **A role is a fixed slot, a board is a name** (44). Both roles are rows whether or not a board holds one — nothing else renders "DUT: not enrolled" — plus a row per board enrolled outside the pair, flagged *not a role* and retractable, since Core now refuses to create another. **An occupied role accepts a drop**, on a dialog pre-filled from the enrolment it names and replaces (43). What a board *is* — chip, west target, note — is the open repo's `embarch/boards.toml`; **a name that does not resolve there renders as unresolved, never as absent**.
...
- **Validate topology says what it checked and what it cannot.** A role is a live hardware-ID re-read; **a guessed dev-bench port is a warning, never a pass**; a signal route is checked only as far as declared, and each line says no software can see the wire. Core's reason is printed verbatim.
