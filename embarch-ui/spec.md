...
| **Study Designer** | Authoring a study: steps, the two `requires` fields, security levels, GATT capture taps and their record framing, `RunProtocol` steps against the repo's `.eap` manifests, the dev-bench log level, the declared-GATT picker, the custom-action registry and payload layouts (author, edit, delete), an `.eap` text editor, and running a saved study as it is on disk |
...
- **A limit enforced server-side is *served*, never restated in `app.js`** — and since `suite/017`, so is a **vocabulary**. The Study Designer's built-in action picker renders the entries and labels the crate serves ([`embarch-study-designer` decision 73](../embarch-study-designer/decisions/authoring.md)); it used to hold its own hardcoded array of nine, beside a served list it filtered out and discarded, and the discarded one had been two variants stale since that crate's decision 53. The eighteen scalar types, the five log levels — **with which one is the default, and the prose under each** — the four caps and the three advisory dev-bench capacities are served the same way, and a test asserts `app.js` holds no copy of any of them. **An empty served vocabulary is an empty picker, which is a refusal**, never a guessed list.
- **An advisory capacity never gates.** The three dev-bench caps are read from a build that may not be the bench in front of you, so a study past one of them says so and Run stays enabled. **A capacity this tab could not read renders as *unknown*, never as "within caps"** — and two of the three cannot be computed in a browser at all (a postcard wire length, and the event arms of a resolved `ProtocolDef`), so `POST /preflight` builds the whole list server-side.
- **A destructive edit that would break a saved study is refused, naming the studies.** Deleting or renaming a registered action, a payload layout or an `.eap` file scans `embarch/studies/` first and answers `409` with the studies and the steps in them. **A file the scan cannot read or parse is reported as unscannable, never as "no references"** — a directory it cannot fully read is not permission to delete. The refusal leaves the form filled, because a refusal that discarded it would make the retry a retype.
- **A capture that was not checked never reads as clean.** `StreamRef.records` absent means the study declared no record framing, which is a different fact from "every record verified"; an empty capture reads "nothing to check" rather than verified, which is why the browser deliberately does not use `RecordReport::all_verified()`.
