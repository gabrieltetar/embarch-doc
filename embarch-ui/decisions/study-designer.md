...
**Reflash lives in the run dialog, never in the saved study** — the study is the experiment, the run dialog is this particular run, the same split decision 10 makes for signal routes. A mismatch is shown *before* the run with both strings, so the choice is made against the actual discrepancy rather than in the abstract. **Unreadable is rendered as unreadable, which is not a mismatch:** a bench that is not plugged in has no version to disagree with, and presenting that as a discrepancy would be a claim about a gap nobody established.
