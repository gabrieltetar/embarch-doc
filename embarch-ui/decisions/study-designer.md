...
**Its cost:** the number is clamped to `total_steps`, because after the last step lands a one-poll window (5 s) has Core still reporting `running`, where the honest ordinal is `3/2`. So a run's last seconds read `2/2` with nothing running — bounded, brief, ending in `completed`. A zero-step study gets **no counter** rather than `1/0`.
