...
**The diff tolerates a growing trailing line too** — Core's tail can end mid-line. `diff_new_lines` (`src/logs.rs:126`) needs exact overlap; a mutating last line never matches, so the same no-overlap fallback fires as for volume aging the window out.
