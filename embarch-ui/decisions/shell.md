...
**The same tracer emits the whole mark as a standalone SVG, outline and all** (`assets/brand/embarch-mark.svg`). Its `layers` mode adds a third path — the dark stroke as the entire silhouette with each letter's own pixels over it, which is how the master is built — so this one reproduces the logo rather than approximating it, and keeps the master's box and padding because an app icon's breathing room is part of the artwork. It is served at `/favicon.svg` ahead of the PNG, which stays as the second `<link>`: an SVG icon is the one asset type a browser may decline. **Tolerance 1.6 is the glyph's number and the sweep says it is the right one.** That layers-mode trace settles at 22 vertices for the E (plus its 4-vertex counter) and 15 for the A (plus its own 4-vertex counter), 657 B inline. **The raster icons then come off that vector rather than off the master** — the favicon and the VS Code extension's icon are the same paths scan-converted at 16x and box-filtered in linear light — so the mark has one source instead of a vector and two independent downscales that could drift.
