...
- **No build has been run from the UI.** `tests/browser/drive_build.py` drives both halves of the Build card — it asks the survey which bench it is on — and against this bench's own workspace the pickers fill from a live west scan and the ordered list demonstrably reorders. What that does not touch is the sequence behind the toggle: check, build, flash, reset, and Core's own mode pre-flight. **Trigger:** one run with the toggle on, which is also what unparks `tasks/core/094` and `tasks/study-designer/068`.
