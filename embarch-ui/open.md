...
- **Where the reflash selector should live is genuinely undecided.** `run_study --reflash` is `embarch-api` orchestration; `embarch-ui` posts studies straight to Core and builds nothing. Three shapes: duplicate the orchestration here, depend on `embarch-api` (a direction the suite has nowhere), or leave reflash terminal-only. Settled as the third for now; the run dialog's string was read and does say so — a limitation, not a design goal. Decision 11.

- **Where the trace analysis lives is settled and it is not here** ([suite decision 4](../suite/decisions.md)). The timeline and the load repartition go to `embarch-core` — on both paths, and shipped by the archive, which the bullet above says this is not; the chart geometry and decision 18's binning stay. **Nothing here is wrong today:** the move is queued (`tasks/core/057` first, the `embarch-ui` half last) because the gap is on the agent path, not this one.
