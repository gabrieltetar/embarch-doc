...
- **The available half of the Build card is driven on the bench, not in a script.** `tests/browser/drive_build.py` covers the unavailable path — no project, no `embarch-api` config, nothing built — because that is the state a fresh checkout is in. Exercising the picker, the ordered snippet list and a real build needs a configured project and a west workspace, so the ordered-snippet control and the three-state mode picker are asserted by nothing automatic. **Trigger:** a bench run with the toggle on.
