...
- **Settled as split-for-now, decision 27 — checked against `embarch-core`'s spans route and still blocked, precisely.** `embarch-core` shipped `GET .../load/spans` (`tasks/core/076`), and `ui/065` compared its payload to `trace.rs` field-for-field rather than swapping code in on faith. It falls short three ways: Core's `Gap` omits `records_lost`/`row_index`/`unbounded_start`, which `app.js`'s gap table renders today; none of the axis-health diagnostics (`frames`, `resolution_ms`, `dual_clock`, `dut_clock_refused`, the stale-prefix and out-of-order counts) are served; and point events are excluded by the same decisions and built in the same row pass as `Lane`/`Span`/`Gap`, so the row decode and clock-health/stale-prefix search can't be deleted while markers still need them. Not this crate's fix — widening `embarch-core`'s `Gap` or diagnostics is filed to `inbox/`, not decided here.
