...
- **Settled as split-for-now, decision 27.** `ui/051` moved the aggregate off this crate ([`embarch-core` decision 62](../embarch-core/decisions.md); [suite decision 4](../suite/decisions.md)); the rollup route carries no per-span data, so the row decode, clock-health/stale-prefix search and `Lane`/`Span`/`Gap` construction (all four exclusion flags) stay duplicated in `trace.rs` and `outpost_load.rs` — wider than this bullet used to say. Unblocks if `embarch-core` serves decoded per-lane spans; filed there, not decided here.
