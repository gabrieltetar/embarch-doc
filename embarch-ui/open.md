...
- **A flash or a reset that fails mid-run has never been watched.** The build-failure branch was driven on hardware 2026-09-19 with a deliberate syntax error and is closed; what is still only unit-tested is the two steps after it — a `POST /flash` Core refuses, and a reset that does not take. Both would surface as a `failed` frame at phase `build` with a Core error in it, and neither has been seen. **Trigger:** unplug the DUT's probe between the build finishing and the flash starting, which is the cheap way to produce the first one.
