...
| [decisions/topology-tab.md](decisions/topology-tab.md) | 10 (routing), 43, 50 | Signal routing: the one human surface for declaring a wire; enrolling a board by dropping a probe onto the diagram, the Enroll tab folded in; and the tab's explanation paragraphs removed, a path and a state being the only notes that stay |
