...
Core creates the directory with a plain `create_dir_all` and locks down only the **file**, with **owner-restricted permissions**. On Windows that means **explicitly granting only the creating account, `SYSTEM` and Administrators rather than relying on `ProgramData`'s default inheritance, which is more permissive than a user-profile directory** — done by shelling out to `icacls` rather than through an ACL crate, **a thin and infrequently-updated wrapper worth avoiding for a security-relevant, Windows-unvalidated operation.** The `icacls` call names the file path only; the parent directory keeps whatever default ACL Windows gives a fresh `ProgramData` subfolder — this doc does not assert what that default concretely grants on any given machine, only that Core **deliberately** never restricts it — [`embarch-core` decision 53](embarch-core/decisions/auth.md). That permissiveness is what `embarch-topology` decision 23 relies on for its `enrollment.toml`, one level down in the same directory, to be readable and writable by both the Core service account and an unprivileged interactive CLI — **and that consequence is an inference from "Core never restricts the directory", inherited from decision 23 and never checked against a real ACL by anything in this suite.** Core's side of it is code and is stated above; the two-account conclusion is not, so do not cite it as measured.
