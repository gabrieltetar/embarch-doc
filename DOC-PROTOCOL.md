...
- 2026-08-31 — **§7.1 and §7.5 applied rather than merely stated, at the repo owner's direction.** `draft` is gone from the repo: 28 status lines swept, 16 → `active`, 11 → `done` (each carrying the closure it claims), and [embarch-umbrella/milestone-6.md](embarch-umbrella/milestone-6.md) deliberately held at `active` because its own Definition of Done is unmarked — a `done` there would be the status claim §4 exists to prevent. §7.5's brackets went onto the two constant inventories that exist ([embarch-study-designer/decisions.md](embarch-study-designer/decisions.md)'s `limits` table; [embarch-outpost/design.md](embarch-outpost/design.md) §5.3's Kconfig table, which also gained the two symbols its own §7 had measured into existence and never listed), plus the three scattered sites where vague hedging actually attached to a constant. §7.5 now states that scope, because applying it is what revealed that a bracket on well-derived prose is noise: 33 `[assumed]` and 17 `[measured]` repo-wide, and the sweep found only five vague-hedging sites to convert, not the dozens the rule's wording implied.
