...
- **Run the checks.** `scripts/check-docs.py` runs the whole gate in one command. **It and CI do not run the same set, and the difference is deliberate**: CI additionally asserts `suite/features.md` is assembled, which no branch can pass; the wrapper additionally runs `install.py --check` and `check-client-names.py`, neither of which CI can — one needs the framework repo beside this one and the other needs a denylist that is not in any repo. Run the wrapper and read what it prints; do not restate its list anywhere, which is how it came to say six while running eight. `collect-open-questions.py` and `queue-status.py` are read-only indexes, not gates.
