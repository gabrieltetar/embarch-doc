...
- **Update suite-level facts in the same pass** — *unless you are a background worker, which drops a `status.d/` fragment instead ([embarch-parallel-agents.md](embarch-parallel-agents.md) §9)* — [embarch.md](embarch.md) §3's status table, a [roadmap](embarch-roadmap.md) bucket, a [features](suite/features.md) row. A sub-project doc and the suite-level docs must never disagree about status.
- **Link, don't restate.** [suite/features.md](suite/features.md) and [embarch-roadmap.md](embarch-roadmap.md) point at a decision rather than duplicating it. Exception: [embarch-user-guide.md](embarch-user-guide.md), where a getting-started guide that only links is useless.
