...
**The reason this doc used to give was wrong.** It said the VS Code extension runs no status line. One *is* configured — `statusLine` in `~/.claude/settings.json`, running `~/.claude/statusline-usage.py` at `refreshInterval: 60` — and given a payload containing `rate_limits` it writes the cache correctly [verified 2026-09-03 against a sandboxed `HOME`]. What is missing is `rate_limits` in the payload, not the status line.

**The two are indistinguishable on disk, which is how the wrong reason survived.** `write_cache` returns early unless `rate_limits` is a dict, so "never ran" and "ran with no numbers" both leave no file; narrowing further needs a payload capture. The script's docstring names the candidates: `rate_limits` arrives only for a Pro/Max seat, only after a session's first API response, and each window disappears once its `resets_at` passes — so an unconstrained window is *expected* to report nothing.

None of that changes what a leg does: no numbers, so DEGRADED, so `--check-429` is the real protection. `statusLine.refreshInterval` stays **required** — the event-driven triggers go quiet exactly while a session waits on background subagents, which is what a supervisor does.
...
**A dream post carries `crystal_ball` as well as `robot_face`, and that is what the gate reads.** The original rule looked for "a `robot_face` dream post", but the connector posts as the owner so *every* fleet message carries it — leaving the gate unfalsifiable both ways: able to read a unit line as a dream, unable to exclude a real second one. One extra reaction fixes that without inventing a state file, and keeps §5.2's rule that reactions are the watermark.
