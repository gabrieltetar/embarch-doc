...
**On this machine the percentages never arrive, and that is the normal case.** Quota state arrives over the wire, so only a status line can see it — and the VS Code extension does not run one, with no `claude` CLI here to run one either (§3). A transcript carries token counts and `429`s but no quota state. (Where a status line *does* run, `~/.claude/statusline-usage.py` caches the percentages for `scripts/usage-budget.py`, and `statusLine.refreshInterval` is **required**: the event-driven triggers go quiet exactly while a session waits on background subagents.)
...
**Push rarely** (`agentPushNotifEnabled`, `inputNeededNotifEnabled`, both on, skipped while the owner is at the terminal): leg blocked and stopped, budget HOLD, or a `suite`-scope design about to execute. **Never per unit, and never on an ordinary leg end** — legs end every twenty minutes, and a push each time is a pager.
...
**Announce and park — never announce and block** (§3 is why: a blocking question freezes a leg with workers in flight). Before starting a `suite`-scope task, or any change that bumps a wire schema version, the supervisor posts to **#embarch-fleet** (`C0BUKTL2FPC`, §5) saying what it is about to do, which repos it touches, why, and that a reply cancels it. It records the message's `ts` **in the task file**, not only in its head — a leg is four units long and the window is thirty minutes, so the `ts` routinely has to outlive the leg that posted it. Then it **does not start that task**: it keeps running units normally, so single-repo workers are not delayed by a decision that has nothing to do with them.
...
**The heartbeat only fires while the listener is idle, and that is load-bearing.** `CronCreate`'s own contract says jobs fire only while the REPL is idle, never mid-query — so the fallback is unavailable for exactly as long as a tick is running. The consequence is not obvious and cost five hours on 2026-09-03: **a tick that retries a failure internally suppresses its own retry.** A leg died on HTTP 529, the leg-death wake-up tried to respawn, the respawn died too, and the fleet sat dark with the pump latched on and work in the queue until the owner poked it by hand. So [fleet.md](.claude/commands/fleet.md)'s STEP 2 makes **one** spawn attempt and then ends the turn: a tick that gives up returns the session to idle and is retried in about eleven minutes, and keeps retrying until the API recovers. A tick that keeps trying is the fleet disabling its own recovery.
...
**That diagnosis was right and its scope was drawn too narrowly.** "Cron went quiet for its duration" is not a property of running a batch inline — it is a property of the listener being mid-query at all, from any cause. So a stuck or hanging tick strands a `fleet stop` in precisely the same way the inline batch did, and for the same reason: a listener mid-query cannot read the channel. The unit-boundary poll covers a *live* supervisor; nothing covers a wedged listener, and closing VS Code is the backstop for that ([the risks](embarch-fleet-risks.md)).
...
- **A leg's refill finds nothing dispatchable**: post three proposals, mention `<@U0AGQGSHM2P>`, and **end the leg**. Do not pick one.
...
**A dream post carries `crystal_ball` as well as `robot_face`, and that is what the gate reads.** The original rule looked for "a `robot_face` dream post" — but the connector posts as the owner, so *every* fleet message carries `robot_face` and nothing marked a dream as a dream. The gate was therefore unfalsifiable in both directions: any leg-start or unit line inside the window could be read as a dream and block a spawn that should happen, while a real second dream an hour later could not be excluded. One extra reaction fixes it without inventing a state file, and it stays consistent with §5.2's rule that reactions are the watermark.
