...
- **[embarch-core](../embarch-core/design.md)** — opens the tap for the study's duration, writes `streams/`, decodes against the manifest, stores manifests keyed by CRC, and serves the results. Core is the only process that touches the serial port, exactly as it is for the dev-bench link and for `/serial-log`.
