...
**The trade is latency, and only latency — bought back by the per-record clock.** A record can sit in the ring for the wait before transmission, **and that costs nothing because it carries its own DUT stamp, so when it left the ring has no bearing on when a host says it happened. Under layout 2 this change would have been unavailable**: a frame's arrival was the only clock, **so delaying a frame would have moved every event in it.**
