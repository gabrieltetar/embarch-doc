...
- **The Zephyr half of this repo's tests is still covered by nothing mechanical.** The suite-scope CI call this bullet used to leave open was settled 2026-09-11 as [suite/decisions.md](../suite/decisions.md) 2: `host-tests.yml` now runs `decoder_unit.py` and `vocab_check.py` on every push and PR, and the three `west`-dependent legs plus the cross-decoder are stated there as deliberately uncovered, with the reversal condition that brings them in. **Read a green check as the host decoder and the record vocabulary agreeing with `src/outpost_priv.h`, and nothing about the module as Zephyr builds it.**
