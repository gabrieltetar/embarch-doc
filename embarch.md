...
| `embarch-dev-bench` | Physical rig that plays the DUT's BLE counterpart (advertise/connect/GATT exchange) and samples power during a `Study`, per `embarch-study-designer`'s types — Zephyr-based C firmware, one shared application spanning multiple vendor-specific west workspaces | First implementation pass at [gabrieltetar/embarch-dev-bench](https://github.com/gabrieltetar/embarch-dev-bench): shared `app/` (serial protocol, stubbed FFI, BLE bridge real+stub); `native_sim` workspace built and verified end to end (`west build` + a live Hello/HelloAck/demo-sequence run + all ztest cases passing); `nordic`/nRF54L15DK workspace is a manifest-only skeleton, not yet built against real hardware | [embarch-dev-bench/design.md](embarch-dev-bench/design.md) |
