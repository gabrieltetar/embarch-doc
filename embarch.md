...
- **Shipped milestone docs and implementation guides are deleted, not indexed** (2026-09-02, [DOC-PROTOCOL.md](DOC-PROTOCOL.md) §3): 22 of them, 334 KB, folded into their sub-project's own docs. A `milestone-N.md` named in prose elsewhere is one of these — recoverable with `git show <rev>:<path>`. None remain: the last one closed into [embarch-umbrella/decisions.md](embarch-umbrella/decisions.md) and [embarch-umbrella/open.md](embarch-umbrella/open.md).
...
- [embarch-umbrella/decisions.md](embarch-umbrella/decisions.md) — embarch-umbrella design (setup/verify/start-Core tooling; the `embarch` binary)
