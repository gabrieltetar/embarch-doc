...
4. **Land.** As each worker reports, run the gate (§10) independently — do not trust the worker's word for green — then merge both of its branches in the order §10 fixes. Rebase the rest onto the new `main`. **A rebased branch's tip is never an ancestor of `main`** even when its content landed, so `merge-base --is-ancestor` cannot prove a rebased branch is safe to delete — check the commit it rebased *to*. Batch 003 correctly refused to delete one on this basis. **Record every merge commit's SHA**: under §6 there is no merge commit and no branch name left afterwards, so the SHA is the only handle a revert has (§11). Delete a worker's worktrees once its branches have landed or been abandoned.
...
- **The owner's session and the supervisor were the same context until 2026-09-03.** It ran batches, listened to Slack, and wrote the rules the supervisor obeys — amending standing rules and `scripts/` repeatedly, legitimately, with nothing structural marking the boundary. The rules the fleet runs under were authored by the context that runs under them, minutes apart, which is how three "documentation shaped exactly like the data it documents" defects shipped and were then caught only by the machinery they broke. Split by [ops](embarch-parallel-agents-ops.md) §8.1: the supervisor is now a disposable agent and `check-ownership.py --supervisor` rejects the reserved paths mechanically. **The residue is that a batch run inline — which the owner may still ask for — puts them back in one context.**
