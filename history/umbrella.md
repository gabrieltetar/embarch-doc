...
- `doctor`'s one-line message rule is now stated as a runtime property, with the live check-1 counter-example — [decision 43](../embarch-umbrella/decisions.md).
...
- `doctor` check 8 asks `embarch-api list-targets` instead of umbrella's own scanner, which is deleted: [decision 17](../embarch-umbrella/decisions.md).
...
- `doctor` check 17 withdraws `embarch setup` where it would reinstall the narrow bind, and no longer judges a `remote` Core by this host: [decision 22](../embarch-umbrella/decisions.md).
...
- `doctor` check 1 finds the Windows service's own Core on `wsl-host` instead of failing, so check 14 runs there: [decision 38](../embarch-umbrella/decisions.md).
...
### Decided
- Decision 55 moved verbatim from `decisions/projects.md` to new `decisions/serial-port.md`.
- `init` never writes `serial_port`, deliberately (decision 55): discover it per call via `list_serial_ports`.
