...
### Added
- `Alert` and `DetectedPort` gain `PartialEq`, plus a `wire`-only test module so the deserialization contract is exercised without `hardware`.
...
### Decided
- Decision 32 closed: `embarch-core::resolve_probe` now calls `select_probe` too (`core/055`); no probe-selection copy remains in either crate.
