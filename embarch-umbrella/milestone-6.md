...
### 3.7 Release engineering — **done, 2026-08-11 for embarch-core/embarch-api; embarch-umbrella's own tag not pushed yet**
...
`assemble-suite.yml` is deliberately manual (`workflow_dispatch`, three release-tag inputs plus a suite version), not chained automatically off any one component's tag push — which three versions form a version-tested suite release is a decision, not a mechanical consequence of one repo pushing a tag. **Not yet run**: it needs a real `embarch-umbrella` release tag too, and pushing that tag hit a wall this session couldn't get past — the coding agent's own push kept being denied by a local permission classifier for that one repo specifically (`embarch-core`'s and `embarch-api`'s identical `git push origin v0.1.0` both went through). Needs a human to push `v0.1.0` on `embarch-umbrella` before `assemble-suite.yml` and `doctor`'s manifest check can be verified against a real suite archive rather than a hand-crafted one.
