...
|---|---|---|
| `embarch setup` | once per machine | Detect topology, install Core as a service, ensure the token file exists, copy all three binaries to the canonical per-user location and put it on `PATH`, record the class and the Windows-side Core path, then point at `embarch status` and `embarch init` as next steps — it does not run `doctor` itself. `--uninstall` reverses it; **`--dry-run` prints the whole plan and changes nothing** (decision 21); `--dev-bench-repo` records the checkout check 13 compares against |
| `embarch init` | once per firmware repo | Scaffold the repo's `embarch/` config, exclude it locally, register the MCP server, then point at `embarch status` and the first `embarch-api build` as next steps — it does not run `doctor` itself. **An inferred board is never written as fact, and several recorded builds are all named rather than one picked** (decision 41). `--uninstall` reverses all of it; committing the integration for a team is a follow-on step it does not take (decision 12) |
...
Umbrella invents no token mechanism — [embarch-token.md](../embarch-token.md) is the source of truth — and **writes no token value into any config file.** Of the two same-machine topologies, only `local` actually starts Core, so only there can a clean `setup` promise the machine-wide token file exists; on `wsl-host` — today's primary topology — `setup` prints the elevated command for a human to run and starts nothing itself, so it can finish with the token file still absent. Either way, its token step is a bare `Path::exists()` at the conventional path (`token_path_for`) — an existence check, not discovery. Confirming that `embarch-api` can actually read the token is the doctor chain's job: check 4 and `status` (decision 46) are the only real callers of token discovery, and `setup` is not among them. Across machines there is no shared filesystem and no solution: it prints the export line for a value the human reads off the Core machine.
