...
**Status:** draft, 2026-08-10. Implemented at [gabrieltetar/embarch-umbrella](https://github.com/gabrieltetar/embarch-umbrella): every command in §8's surface has real behavior behind it — `setup`, `init`, `doctor`, `up`/`down`, and `status` all ship. Only §5's release-manifest comparison (check 1) and the `embarch-study-designer` schema-version check (check 11) are still stubs, both for the reason recorded at each: nothing exists yet on the other side of either comparison ([milestone-6.md](milestone-6.md) §3.7 hasn't shipped; `embarch-study-designer` isn't a Cargo dependency of `embarch-core`/`embarch-api` yet). Append changes to the Changelog (§12) rather than silently editing history above it.
