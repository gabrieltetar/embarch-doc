...
**Verified live** [measured 2026-09-06, `embarch doctor` run from two directories on the primary bench]. Both arms, on the mechanism this decision describes — the structural read of the agent CLI's own config, identity by the binary the entry names, local scope before user. From a directory with no entry: **Fail `not-registered`**, *"nothing registered for `/home/gabriel/Github/embarch/embarch-umbrella`, under any name"*. From one with: **Pass `handshake-ok`**, *"registered as `embarch-api` (local scope), and it answered initialize (rmcp)"* — a real spawn and a real JSON-RPC `initialize`.

**What the run did not reach is the residual above**: a server needing something only the agent CLI's environment supplies would still fail here and work there, and neither arm was that. `handshake-ok` proves the registered `env` was enough for *this* server, not that `doctor`'s environment is equivalent to the CLI's.
