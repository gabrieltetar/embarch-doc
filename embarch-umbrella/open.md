...
- **Seven designed pieces are confirmed unbuilt; what is open is whether each is still wanted.** Decisions 18, 21, 22(a-c), 23, 26, 27/29 and 17's amendment, read against the source 2026-09-03: **none is built**. [spec.md](spec.md) and each entry say so, and the work is queued in `inbox/`. **Some may be worth retiring instead**: 26's pruning has drawn no pressure, and 18 is a Linux-first-run fix on a Windows-primary suite. Nobody has weighed that.
