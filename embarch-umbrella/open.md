...
- **Check 10 still spawns the MCP server in `doctor`'s own environment, not the agent CLI's** ([decision 40](decisions/doctor.md)). The registered `env` is applied now that the entry is read structurally, so a server needing one of *those* variables is covered; one needing something the CLI's environment supplies and `doctor`'s does not will fail here and work there. **And the rebuilt check has never run live** — every claim about check 10 before 2026-09-05 was made against a parse of a format that does not exist.
