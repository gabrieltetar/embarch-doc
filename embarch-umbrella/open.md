...
- **Check 13 is silent by default, and its one real comparison was against a history that no longer exists** [2026-09-07 bench run, `tasks/umbrella/034`; the budget debt above is closed]. `dev_bench_repo_path` is unset in saved state and nothing in `setup` or `init` records it, so check 13 warns *"no embarch-dev-bench checkout configured"* with the checkout two directories away; only `EMBARCH_DEV_BENCH_REPO_PATH` produced a comparison, and it is a **`FAIL`** — bench firmware `49958d34` against checkout `d599453d`, and **`49958d34` is in no commit, tag or reflog of that repo**. So it compares `git describe` values across a history rewrite, and reflashing is the only thing that can ever clear it. `tasks/umbrella/037`, `tasks/dev-bench/011`.
