...
**Corrected 2026-09-08 — the reversal was decided and never executed, and this repo has the least mechanical checking in the suite.** `git log --all -- '.github/**'` returns nothing here: **this repo has never had a `.github` directory at any commit.** So the `native_sim` job above is a design, not a description. It is also the sharpest instance of the gap, because `embarch-dev-bench` has no `Cargo.toml`, which means **the fleet's merge gate ([the protocol](../../../embarch-fleet/protocol.md) §10) cannot reach it either** — its `cargo build`/`test`/`clippy` half selects nothing here. Between them, nothing mechanical checks a change to this repo before it lands; the doc-side checks run, and a human or an agent reading the diff is the rest. `embarch-outpost` is in the same position for the same reason. See [embarch.md](../../embarch.md) §5's table. **Still worth building** — this correction retires the claim that it exists, not the argument that it should.
