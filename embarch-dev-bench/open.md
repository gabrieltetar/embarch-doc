...
- **The bench resets mid-transmission, and a truncated `StepResult` is its shadow.** A study step failed, Core refused the frame as badly encoded, dropped the link, and failed the whole study as a *connection error* — **never saying a step had failed at all**, so the one message whose job is to explain a failure was the one that could not get through. Core's side of that is fixed ([embarch-core](../embarch-core/decisions/studies.md) decision 40); the reset itself is not.

  **The proof is an uptime comparison, not a byte pattern:** the link handshook at uptime 899,843 ms and the *next* handshake reported 46,320 ms — placing a reset within 150 ms of the short frame. Everything that looked like a transport fault follows: a byte-perfect prefix with the tail simply absent; a last byte that can be *corrupt*, because the line went idle mid-byte and the remaining bits sampled as idle `1`; nothing following, not even a close; only long frames losing their tail, since short ones finish before the window the crash falls in; and **intermittency**, which is a crash rather than a boundary.

  **"The number that identifies it is 16" was wrong, and that number was the whole basis of the previous diagnosis.** Six captured occurrences short by 15, 17, 17, 13, 17, 13 — not one of them 16 — and a seventh identical frame arrived complete. The original reading came from subtracting a delimiter-inclusive length from a delimiter-exclusive one. **A search for a 16-byte TX boundary rested on it.** What actually needs finding is why the bench reboots; the candidates have not been narrowed.
