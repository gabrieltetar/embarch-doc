...
- **A third instance of the same class is not closed: `enroll`'s probe selection duplicates `embarch-core::resolve_probe` (decision 32).** This time the direction is the other way from the two above — de-duplicating means `embarch-core`'s own `resolve_probe` calling a `pub` helper this crate would expose, since `embarch-topology` cannot depend back on `embarch-core`. That edit is inside `embarch-core`, not this crate, so it stays open here until a `core`-scoped task picks it up.
