...
- **A third instance of the same class is half-closed: `enroll`'s probe selection duplicated `embarch-core::resolve_probe` (decision 32, amended; decision 33).** The topology half is done — `enroll` now calls a `pub` `embarch_topology::hardware::select_probe`, reconciling the three behavioral divergences the two copies had already accumulated. What's left is entirely on the other side: `embarch-core::resolve_probe` still keeps its own copy until `tasks/core/055` (blocked on this landing) replaces it with a call to `select_probe`. That edit is inside `embarch-core`, not this crate, so it stays listed here until that task closes it.
