...
**Closed 2026-09-13 — `embarch-core::resolve_probe` now calls `select_probe` too.** `tasks/core/055` landed (`86345c01a451b0696af36f42c28bf6676478124c`): `resolve_probe` no longer keeps its own copy — it enumerates probes and delegates, threading a caller `action` (`"flash"`/`"reset"`) through the same way `enroll` does (`embarch-core` decision 61). Both sides of the duplication are closed.
