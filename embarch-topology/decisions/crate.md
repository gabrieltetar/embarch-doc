...
**Why the same directory, rather than a directory of this crate's own:** it has to be machine-wide and admin-owned for the same reason Core's token does — **a Windows service and the unprivileged CLI that reads its output are two different accounts, and only a location neither owns exclusively lets both see the same file.** Reusing Core's directory rather than inventing a sibling one is continuity, not coincidence: it is the same directory `known_boards.toml` already used before this crate existed (decision 3), so a store from before this decision still loads without migration.
