...
### 34 — `validate_known_timed`'s five mid-attach failures now raise and log too, not just "not attached" and "mismatch"

Decision 12 says every constructed `TopologyMismatch` is durably logged — true, but until now it was constructed at only two of `validate_known_timed`'s seven failure points (probe absent from `Lister::list_all()`, and a hardware-ID compare that doesn't match). The other five — `probe_info.open()`, `check_target_powered`, `.attach()`, `session.core(0)` and `hardware_id::read`, the whole "open and read the board" sequence once the probe is found — returned a bare, un-logged, non-downcastable `anyhow::Error`. A caller (`embarch-core`'s `POST /validate`, and `flash`/`reset`/the dev-bench gate behind the same downcast) fell through to a generic error arm for all five, indistinguishable from any other I/O failure in the suite.

**Routed, not left silent.** All five now call `raise`, each with a `reason` naming which step failed and why, folding in the underlying error text so nothing an operator would have seen from the old plain error is lost. **The five are not interchangeable and do not share one message** — an unpowered board, a busy or permission-denied probe, a failed attach, a failed core-select and a failed ID read are five different facts, and the case named as most worth this (a probe that lists but won't open — another process holding it, a permission denial, a half-wedged J-Link) gets its own wording rather than a shared generic one.

**What this does not close, stated rather than papered over.** All five newly-routed failures set `live_hardware_id: None` — the same value the pre-existing "not enumerated at all" branch sets — so a `TopologyMismatch`'s structured fields still cannot tell "the probe was never there" apart from "the probe was there and got stuck." Only `reason`'s text carries that distinction now. Widening `live_hardware_id` or adding a new field to say so structurally would be a shape change reaching every consumer that reads a `TopologyMismatch` (`embarch-core` first), which is exactly the kind of decision this crate's own consumer-boundary rule (decisions/consumer-boundary.md) reserves for the consumer that actually needs it — not assumed here on its behalf. `embarch-core`'s `kind: "not_attached" | "mismatch"` classifier (that crate's decision 59) will read all five as `"not_attached"` until and unless that crate decides it needs a third value; that question is filed as its own paired task (`tasks/core/076`) rather than answered here, the same two-repo shape `core/041`'s resolution used.

**Rejected: leaving the five silent, on the reasoning that `embarch-core`'s own classifier could not yet use the extra alerts anyway.** That confuses "the alert log's read side isn't finished" with "the alert shouldn't be logged." Decision 12's whole point is that durability does not wait on a consumer being ready to act on it — a row landing in `alerts.jsonl` the instant a probe-open failure happens is worth having before `embarch-core` ever gets a third `kind` value, not after.
