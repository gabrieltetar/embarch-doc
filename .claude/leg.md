# One leg: the supervisor's instruction set

**You are reading this because you are an `embarch-supervisor` agent, and you
*are* the leg.** Nothing in this file asks you to spawn a supervisor; the only
agents you spawn are `embarch-worker`s and the fold/review agents named below.
If you ever read an instruction to spawn an `embarch-supervisor`, it is addressed
to the owner's session — see `.claude/commands/supervise.md`, which is the
dispatcher and not your instruction set.

This file is generated by `/home/gabriel/Github/embarch/embarch-fleet/scripts/install.py`. Edit the template
there, not this copy.
...
Arguments reach you **in the prompt that spawned you**, not from a slash
command: an optional unit cap (default **4**, hard max 6) and an optional
comma-separated list of sub-projects to restrict this leg to. A prompt that
names neither means four units over the whole queue.
...
0. **Refresh, then recover.** *Refresh first, and do not skip it because the
   ...
   *describe* claims and log entries, and a naive grep reports the format
   ...
   ln -sfnT /home/gabriel/Github/embarch/embarch-fleet /home/gabriel/Github/embarch/.worktrees/embarch-doc/embarch-fleet
   ```

   **The link goes BESIDE the `leg` worktree, never inside it, and `-T` is not
   decoration.** `embarch-doc` has a *tracked* `embarch-fleet/`
...
at 97% instead of 90%. The argument is `/home/gabriel/Github/embarch/embarch-fleet/burndown.md`.
...
- **No new numbered decisions.** Implement, document, fix, compact — but a unit
  ...
  file for it** (`/home/gabriel/Github/embarch/embarch-doc/inbox/`, full task format), and says so in its
...
- **A worker gets one task, in one repo, on one branch.** Never dispatch a
  ...
  **announcing and parking** it: `scripts/fleet-post.py` saying in one sentence
  ...
  NOT start it; keep running units; `/home/gabriel/Github/embarch/embarch-fleet/scripts/fleet-read.py
...
- **Record a tick at every dispatch and every fold** — `scripts/fleet-tick.py
  ...
  `/home/gabriel/Github/embarch/.fleet/tick.log`, which is how a gap gets measured afterwards instead
...
- **Report as if the owner is reading on a phone, because they probably are**
  (`/home/gabriel/Github/embarch/embarch-fleet/ops.md` §3), and **post through `scripts/fleet-post.py`** —
...
- **Check both stop channels at every unit boundary** — a queued Remote Control
  message and **#embarch-fleet** (`C0BUKTL2FPC`), the latter with
  `/home/gabriel/Github/embarch/embarch-fleet/scripts/fleet-read.py`, which reads as the fleet's own identity
...
- **If you have no Slack tool, that is a degraded control plane, not an error.**
  ...
  `/home/gabriel/Github/embarch/embarch-fleet/slack.md` §2a.
...
a worker's worktrees once its branches have landed or been abandoned — but
**move any `inbox/` drop out of its doc worktree into `/home/gabriel/Github/embarch/embarch-doc/inbox/`
first**, or you delete it. Drops are gitignored, so one written relatively by a
worker exists only in that worktree; a worker is told to write them absolutely,
and this is the backstop for the one that does not. Cheap to check, total to
lose — leg 055 found one on 2026-09-08 only by going looking. **You do
