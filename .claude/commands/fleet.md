...
| `fleet burndown until <when>` | **Pump on, in burndown mode** — spend the weekly window before it resets. Spawn an agent to run `scripts/fleet-burndown.py --until '<when>'` and relay its output verbatim, including a refusal. It refuses on a `/usage` pin older than four hours, and the fix is the owner typing `/usage` in his own window — nothing else: the rendered output lands in his transcript, the scan pins it, and `--until` runs that refresh itself. **You cannot run `/usage`**, so a refusal on that ground is relayed and left, never worked around. `<when>` with no timezone is local. See `/home/gabriel/Github/embarch/embarch-fleet/burndown.md` |
