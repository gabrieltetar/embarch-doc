...
`/home/gabriel/Github/embarch/embarch-fleet/slack.md`. Channel: **#embarch-fleet**, id
...
4. Say so in the channel with `/home/gabriel/Github/embarch/embarch-fleet/scripts/fleet-post.py` — that the
...
> **Fleet tick.** Run, in `/home/gabriel/Github/embarch/embarch-fleet`, exactly:
> `scripts/fleet-read.py`. It reads `#embarch-fleet` (`C0BUKTL2FPC`)
...
> narrating the owner's own app reinstall, authored by `U0AGQGSHM2P`,
...
> 2. Act on it per the vocabulary in `/home/gabriel/Github/embarch/embarch-doc/.claude/commands/fleet.md` —
...
>
> **Then, before spawning anything else: if `/home/gabriel/Github/embarch/.fleet/pending-deploy`
...
> `.claude/leg.md`, read the newest `/home/gabriel/Github/embarch/embarch-fleet/supervisor-log.md` entry as
...
>
> Read with `/home/gabriel/Github/embarch/embarch-fleet/scripts/fleet-read.py`, post with
...
#embarch-fleet at **every unit boundary**, so a stop posted mid-leg lands
...
including the fleet's own unit lines — was authored by `U0AGQGSHM2P`, and
...
| `fleet burndown` | **Pump on, in burndown mode** — spend the weekly window before it resets, with the deadline taken from the pinned weekly reset instant. Spawn an agent to run `scripts/fleet-burndown.py --arm` and relay its output verbatim, including a refusal. It refuses on a `/usage` pin older than four hours, and the fix is the owner typing `/usage` in his own window — nothing else: the rendered output lands in his transcript, the scan pins it, and arming runs that refresh itself. **You cannot run `/usage`**, so a refusal on that ground is relayed and left, never worked around. It also refuses early in a week, when nothing is about to expire. See `/home/gabriel/Github/embarch/embarch-fleet/burndown.md` |
| `fleet burndown until <when>` | Same, with the deadline stated by hand instead of derived — `--until '<when>'`, and no timezone means local. Only needed when he wants to stop short of the reset |
