...
- **A capacity limit refuses; it never truncates.** Every bound is disclosed, and exceeding one is a named error **naming the bound actually exceeded** — not a nearby one that happens to hold the same number.
