...
- **Whether `MAX_DISCOVERED_SERVICES = 8`/`MAX_CHARS_PER_SERVICE = 16` are actually sufficient** — sized against `reference-dut-fw`'s current `ble_def.h` (2 services, largest at 8 characteristics) with headroom; not yet validated against what `GattDiscover` reports live once §3.6's extraction and `embarch-dev-bench/milestone-9.md`'s live discovery can be diffed against each other.

## 6. Changelog

- 2026-08-20 — **§3.1–3.8 executed; this crate's own half of Milestone 3 is done.** New `src/gatt.rs` (`GattCharacteristicInfo`/`GattServiceInfo`/`GattActivityRecord`); `Action::GattDiscover`/`GattMonitorAll` and `StepResult.gatt_services`/`gatt_activity` (both `#[serde(default)]`); `DataChannel::GattActivity`; `STUDY_DESIGNER_SCHEMA_VERSION` 3→4; new `limits` constants. Round-trip tests (postcard + a new `serde_json` dev-dependency) cover every new type/variant, including a legacy-JSON-missing-both-fields case. `GattConfigExtractor` trait + `ZephyrBleDefExtractor` in a new `gatt-extract`-feature-gated `src/gatt_extract.rs` (regex text-scan, `std`-only), plus the `extract-gatt-config` CLI bin (`tools/extract_gatt_config.rs`). §3.6's own DoD line — "confirmed against the real, current checkout, spot-checked by hand" — was run for real: against the `bugfix/gpio-debounce-ppg-work-queue` worktree (the DUT branch now in scope per the design pass that opened this milestone having since been superseded by a real fix landing — see `embarch-dev-bench/milestone-9.md`'s own changelog; `lib/ble/` is byte-identical to `main` on that branch, so this doesn't affect the extraction result), output hand-verified byte-for-byte against `ble_def.h`/`ble.c`: 2 services, Sensor Data Service (7 characteristics, all NOTIFY) and Device Management Service (8 characteristics, mixed read/write/notify/indicate) — every UUID and properties byte matched. That real run caught a design.md decision 15 inaccuracy (it had attributed "8 characteristics" to the Sensor Data Service; it's actually the Device Management Service) — corrected there. `cargo build`/`test`/`clippy --all-targets --all-features -D warnings` clean, and `cargo test` individually clean across `default`/`std`/`core-validation`/`ffi`/`gatt-extract`. `design.md` §3 decisions 31/32/33 updated to reflect this (§3.8). Not this doc's scope, carried to the companion docs: `embarch-dev-bench`'s live BLE dispatch of the two new `Action`s, and `MAX_GATT_ACTIVITY_RECORDS`' real-traffic validation (§5's own open items).
- 2026-08-19 — Initial draft, scoping this crate's half of Milestone 3 (Study Designer: Feature-Branch Iteration): implement `design.md` §3 decisions 31/32/33 in code, none of which exist yet as of this milestone's own design-questions pass.
