...
| `requires: Requirements { dev_bench_version, firmware_version, build, outpost }` | The builds this study is meant to run against. The two versions are mandatory, `any` a legal explicit value. **No serde default on `requires` itself, deliberately:** an omitted one fails to deserialize rather than defaulting to `any`, which is what makes "it has to be *said*" true rather than aspirational. `build: Option<BuildSpec>` and `outpost: Option<OutpostModeRequirement>` (decision 77) each **do** default to `None`, which is what a study authored before them always meant |
| `BuildSpec { board, variant, revision, app, snippets[], extra_args[] }` | The DUT firmware a study builds and flashes for itself. All `Option`/empty: an absent axis takes the project's `default_target`, an empty `snippets` its `default_snippets`. **`snippets` is ordered, not a set** — west applies `-S` in order. No project name and no `build_id`: the run supplies the repo, and the selection is resolved every time |
| `OutpostModeRequirement { required_set, required_clear }` | Two `u8` masks over `OutpostHeader.flags`. Satisfied when every `required_set` bit is set and no `required_clear` bit is. **Two masks because a flag's clear state can be the requirement** (`TRACE_SELF`). A bit in neither is not cared about; a bit in both is refused |
...
Split out to [result-types.md](result-types.md) on 2026-09-13 (`tasks/study-designer/038`) — `Provenance`, `StudyResult`, `StepResult`, and the retired result fields.
