...
`rx_utc_ms` is stamped **by dev-bench at capture time** off its own clock — not assigned by Core on arrival, so the timestamp reflects when the sample was taken rather than when it reached Core over a variable-latency link.

**And that clock is the bench's uptime, not UTC, despite the field's name** (decision 72). The seed-and-resync from `Hello.host_utc_ms` that would make it UTC is designed and **not implemented in firmware**: `embarch-dev-bench` stamps `k_uptime_get()` directly and applies no offset anywhere. So the value is milliseconds since the bench last booted, and it is **not comparable** with `core_rx_utc_ms` in the same row, with any other `*_utc_ms` field in this suite, or with an outpost trace's own `rx_utc_ms`, which *is* Core's epoch clock. Use `core_rx_utc_ms` for anything that has to sit on a real timeline; use `rx_utc_ms` for intervals within one capture, which is the only thing it can answer. `unit` and `channel_id` are declared **once per tap** rather than repeated per message, since neither changes mid-capture.
