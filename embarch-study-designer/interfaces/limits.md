...
| `MAX_DISCOVERED_SERVICES` | 8 | `GattServiceInfo` per `StepResult.gatt_services` (decisions 31/32, §4.3a) | decision 57's validated GATT table: `reference-dut-fw` declares 3 services, 7 in total once an encrypted link reaches the rest (same DUT as `MAX_MONITOR_TARGETS` below), transcribed from `src/limits.rs:80-84`, whose comment was written 2026-08-23 — with headroom for a DUT this crate hasn't seen yet |
