...
**`StreamRef` deliberately did not grow an unstructured `note` field**, and the reasoning is worth keeping: free text with no defined shape is not a fact a caller can rely on the way it relies on `truncated` — it would have been decorative, and Core exposes that kind of ad hoc thing over its own index instead.

**It later grew a fourth field anyway — `records: Option<RecordReport>` (decision 70) — and that is not the same answer reversed, because it is not the same question.** `RecordReport` isn't prose: it's a structured verdict against the CRC-32 each record already carries on the DUT, computed the same way every time from framing the engineer declares per-tap in `Study.record_checks`, so a caller reads it exactly like `truncated` rather than parsing anything. And `truncated` still stays the one thing this type says about the *link* — did something report losing bytes; `records`, when present, says whether the bytes that arrived are the bytes the DUT checksummed, end to end. That is **the end-to-end statement `truncated` cannot make**, not a second way of making the same one, which is why it earned a field here rather than a trip through Core's index: a result consumer judging a capture's completeness needs both facts together, not one inline and one behind a separate route. Growing this type still costs a host schema bump — `HOST_TYPE_SCHEMA_VERSION` 17 → 18, dev-bench's own wire schema untouched — the type just stopped refusing to pay it once the fact was this rather than commentary.
