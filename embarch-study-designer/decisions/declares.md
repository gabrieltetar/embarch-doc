...
**What building it would take:** a `DeclaredGatt` enum (`Vendor` / `Extracted { repo, revision }` / `Authored`) reusing `GattServiceInfo`/`GattCharacteristicInfo` from [interfaces/gatt-types.md](../interfaces/gatt-types.md), a `gatt: Option<DeclaredGatt>` field on `Study`, and the reconciliation pass in Core this paragraph describes. Deferred with no trigger fired yet — see [../open.md](../open.md).
